...
async def test_phase_relation(master_data: AsyncSession) -> None:
    ...
    second_phase = first_phase._create_next_phase()
...
async def test_phase_year(master_data: AsyncSession) -> None:
    ...
    assert second_phase is not None
    assert second_phase.year == 1901
    third_phase = second_phase._create_next_phase()
    fourth_phase = third_phase._create_next_phase()
    fifth_phase = fourth_phase._create_next_phase()
    sixth_phase = fifth_phase._create_next_phase()
    assert sixth_phase is not None
    assert sixth_phase.year == 1901
    seventh_phase = sixth_phase._create_next_phase()
